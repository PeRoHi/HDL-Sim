...
6. **保存の正**: `.spj` 保存時は `verilog_sources/[プロジェクト名]/` へ生ソースを書き出してください。参照元 `source_path` へのサーバー自動上書きはしないでください。ブラウザの File System Access で開いたローカルファイルだけ、ユーザー許可付きで書き戻してよいです。
```
...
- [ ] 保存操作がメタデータだけで終わらず、`verilog_sources/`（または明示したエクスポート先）へソースが書き出されるか。参照元 `source_path` への無断上書きはしないか
